...
- DNS Rescue/Linux: INPUT-chain имена укладываются в 28-символьный лимит
  ...
  Живая проверка также выявила гонку старта: systemd мог уже показать sing-box
  активным до открытия DNS-порта, из-за чего исправный шлюз получал ложный отказ.
  Теперь запуск ограниченно ждёт готовности listener, после чего по-прежнему
  отдельно доказывает ответы UDP и TCP.
- Web UI: `loadStatus()` теперь объявляет снимок DNS Rescue в своей области
  видимости до первого использования. Раньше `ReferenceError` обрывал обновление
  сразу после заголовка и времени: маяк оставался на «Проверяю состояние», а карта
  выхода — пустой. Отдельный regression фиксирует declaration-before-use.
...
- public и canonical: **1203 tests, OK (skipped=3)**; platform-specific Linux-проверки пропущены только на Windows;
...
- локальный canary на VADIM: штатный `UPDATE=1` завершился с `rc=0`, Linux-фокус
  ...
  дерево отката 1.13.0 сохранено без изменений. Исправление статуса панели
  установлено под update/network locks одновременно в runtime и `/opt/redut-src`;
  обе копии совпали с проверенным публичным деревом, новый процесс отдал `/healthz`
  и отрендерил исправленный `loadStatus()`, после чего полный healthcheck снова прошёл;
