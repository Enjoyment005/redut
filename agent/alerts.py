...
class Alerter:
    ...
    def proxy_vanished(self, *, items):
        """Провайдер снял с обслуживания прокси, аренда которого ещё оплачена.

        Письмо не про «панель почистилась» (уборка — рутина), а про деньги: за
        эти дни уже заплачено, и провайдер обязан либо вернуть прокси, либо дать
        замену. items — список dict: uid, host, country, date_end, days_left.
        """
        items = [it for it in (items or ()) if it]
        if not items:
            return False
        lines = "".join(
            "  %s  (%s, %s)\n    оплачено ещё %s дн — до %s\n"
            % (it.get("uid"), it.get("host") or "?", it.get("country") or "?",
               it.get("days_left"), it.get("date_end") or "?")
            for it in items)
        body = (
            "⚠️ Провайдер перестал отдавать прокси, аренда которых ЕЩЁ ОПЛАЧЕНА.\n\n"
            "%s\n"
            "Такие прокси исчезли из выдачи кабинета (адаптеры видят только активные), "
            "поэтому панель их больше не показывает — управлять ими нечем: ни проверить, "
            "ни продлить, ни поставить в бой.\n\n"
            "Что сделать: открой кабинет провайдера и потребуй замену или возврат за "
            "неотработанные дни — деньги за них уже списаны. Если прокси вернётся в "
            "выдачу, панель подхватит его сама вместе с прежней ролью.\n\n"
            "Автоматика уже ищет замену по обычным правилам (§8), вмешательство не нужно."
            % lines)
        return self._send(
            "Провайдер снял оплаченный прокси (%d шт)" % len(items), body)
